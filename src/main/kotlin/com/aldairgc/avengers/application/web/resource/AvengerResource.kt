package com.aldairgc.avengers.application.web.resource

import com.aldairgc.avengers.application.web.resource.request.AvengerRequest
import com.aldairgc.avengers.application.web.resource.response.AvengerResponse
...
class AvengerResource(@Autowired private val repository: AvengerRepository) {
    ...
    @PostMapping
    fun createAvenger(@Valid @RequestBody request: AvengerRequest) = request.toAvenger()
    ...
    fun updateAvenger(@Valid @RequestBody request: AvengerRequest, @PathVariable("id") id: Long) =
        repository.getDetail(id)
            ?.let { AvengerRequest.to(it.id, request)
